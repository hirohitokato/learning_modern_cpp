...
プログラムから送られてくるバイナリデータを何のエンコーディングで表示するか、という話。コマンドプロンプトだと `chcp` コマンド(たぶんCHange Code Pageの略)を使って指定する。

```bat
> type utf8text.txt
... 文字化けした文字列 ...

> chcp 65001 ' 以降の出力をUTF-8で解釈＆表示する
> type utf8text.txt
... きちんと表示される ...
```

コマンドプロンプトのデフォルトは `chcp 932` 相当で、つまりCP932(Shift_JIS)になっている。
...
（https://www.javadrive.jp/command/display/index5.html より）

macOSやLinuxのロケール(LC_ALLとか)は省略。
